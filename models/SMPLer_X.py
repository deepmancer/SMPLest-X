...
# from modules.SMPLestX.models.loss import CoordLoss, ParamLoss, CELoss
# from modules.SMPLestX.smplestx_utils.transforms import rot6d_to_axis_angle, restore_bbox, sample_joint_features, soft_argmax_2d, soft_argmax_3d
# from modules.SMPLestX.human_models.human_models import SMPLX 
...
# import torch.nn as nn
# from torch.nn import functional as F
# from modules.SMPLestX.human_models.human_models_smplerx import SMPLX
# import torch.nn as nn

# from modules.SMPLestX.configs.config_smplest_x_h import cfg

# smpl_x_instance = SMPLX()
...
#         if part == 'body':
#             self.joint_num = len(smpl_x_instance.pos_joint_part['body'])
#             self.hm_shape = cfg["model"]["output_hm_shape"]
#         elif part == 'hand':
#             self.joint_num = len(smpl_x_instance.pos_joint_part['rhand'])
...
#         self.hand_pose_out = make_linear_layers([self.joint_num * 515, len(smpl_x_instance.orig_joint_part['rhand']) * 6], relu_final=False)
...
#             [self.joint_num * (512+3), (len(smpl_x_instance.orig_joint_part['body']) - 1) * 6], relu_final=False)  # without root
#         self.shape_out = make_linear_layers([feat_dim, smpl_x_instance.shape_param_dim], relu_final=False)
...
#         self.expr_out = make_linear_layers([feat_dim, smpl_x_instance.expr_code_dim], relu_final=False)
...
#     def __init__(self, encoder, body_position_net, body_rotation_net, box_net, hand_position_net, hand_rotation_net, face_regressor):
...
#         self.smplx_layer = copy.deepcopy(smpl_x_instance.layer['neutral']).cuda()
#         self.coord_loss = CoordLoss()
#         self.param_loss = ParamLoss()
#         self.ce_loss = CELoss()

#         self.body_num_joints = len(smpl_x_instance.pos_joint_part['body'])
#         self.hand_joint_num = len(smpl_x_instance.pos_joint_part['rhand'])

#         self.neck = [self.box_net]
...
#                                 self.hand_regressor, self.face_regressor]
...
#         else:
#             joint_cam = output.joints[:, smpl_x_instance.joint_idx, :]
...
#         root_cam = joint_cam[:, smpl_x_instance.root_joint_idx, None, :]
...
#         lhand_idx = smpl_x_instance.joint_part['lhand']
#         lhand_cam = joint_cam[:, lhand_idx, :]
#         lwrist_cam = joint_cam[:, smpl_x_instance.lwrist_idx, None, :]
...
#         rhand_idx = smpl_x_instance.joint_part['rhand']
#         rhand_cam = joint_cam[:, rhand_idx, :]
#         rwrist_cam = joint_cam[:, smpl_x_instance.rwrist_idx, None, :]
...
#         face_idx = smpl_x_instance.joint_part['face']
#         face_cam = joint_cam[:, face_idx, :]
#         neck_cam = joint_cam[:, smpl_x_instance.neck_idx, None, :]
...
#                         x = targets[coord_name][:, smpl_x_instance.joint_part[part_name], 0]
#                         y = targets[coord_name][:, smpl_x_instance.joint_part[part_name], 1]
#                         z = targets[coord_name][:, smpl_x_instance.joint_part[part_name], 2]
#                         trunc = meta_info[trunc_name][:, smpl_x_instance.joint_part[part_name], 0]
...
#                         trunc = trunc[:, :, None]
#                         targets[coord_name] = torch.cat((targets[coord_name][:, :smpl_x_instance.joint_part[part_name][0], :], coord,
#                                                         targets[coord_name][:, smpl_x_instance.joint_part[part_name][-1] + 1:, :]),
#                                                         1)
#                         meta_info[trunc_name] = torch.cat((meta_info[trunc_name][:, :smpl_x_instance.joint_part[part_name][0], :],
#                                                         trunc,
#                                                         meta_info[trunc_name][:, smpl_x_instance.joint_part[part_name][-1] + 1:,
#                                                         :]), 1)

#                 # change hand projected joint coordinates according to hand bbox (cfg.output_hm_shape -> hand bbox space)
#                 for part_name, bbox in (('lhand', lhand_bbox), ('rhand', rhand_bbox)):
#                     x = joint_proj[:, smpl_x_instance.joint_part[part_name], 0]
#                     y = joint_proj[:, smpl_x_instance.joint_part[part_name], 1]
...
#                         mask = meta_info['joint_trunc'][bid, smpl_x_instance.joint_part[part_name], 0] == 1
#                         if torch.sum(mask) == 0:
#                             trans.append(torch.zeros((2)).float().cuda())
#                         else:
#                             trans.append((-coord[bid, mask, :2] + targets['joint_img'][:, smpl_x_instance.joint_part[part_name], :][
#                                                                 bid, mask, :2]).mean(0))
#                     trans = torch.stack(trans)[:, None, :]
#                     coord = coord + trans  # global translation alignment
#                     joint_proj = torch.cat((joint_proj[:, :smpl_x_instance.joint_part[part_name][0], :], coord,
#                                             joint_proj[:, smpl_x_instance.joint_part[part_name][-1] + 1:, :]), 1)

#             if not (meta_info['face_bbox_valid'] == 0).all():
#                 # change face projected joint coordinates according to face bbox (cfg.output_hm_shape -> face bbox space)
#                 coord = joint_proj[:, smpl_x_instance.joint_part['face'], :]
#                 trans = []
#                 for bid in range(coord.shape[0]):
#                     mask = meta_info['joint_trunc'][bid, smpl_x_instance.joint_part['face'], 0] == 1
#                     if torch.sum(mask) == 0:
#                         trans.append(torch.zeros((2)).float().cuda())
#                     else:
#                         trans.append((-coord[bid, mask, :2] + targets['joint_img'][:, smpl_x_instance.joint_part['face'], :][bid,
#                                                             mask, :2]).mean(0))
#                 trans = torch.stack(trans)[:, None, :]
#                 coord = coord + trans  # global translation alignment
#                 joint_proj = torch.cat((joint_proj[:, :smpl_x_instance.joint_part['face'][0], :], coord,
#                                         joint_proj[:, smpl_x_instance.joint_part['face'][-1] + 1:, :]), 1)
            
#             loss['joint_proj'] = self.coord_loss(joint_proj, targets['joint_img'][:, :, :2], meta_info['joint_trunc']) * smplx_kps_2d_weight
#             loss['joint_img'] = self.coord_loss(joint_img, smpl_x_instance.reduce_joint_set(targets['joint_img']),
#                                                 smpl_x_instance.reduce_joint_set(meta_info['joint_trunc']), meta_info['is_3D']) * net_kps_2d_weight
            
#             loss['smplx_joint_img'] = self.coord_loss(joint_img, smpl_x_instance.reduce_joint_set(targets['smplx_joint_img']),
#                                                       smpl_x_instance.reduce_joint_set(meta_info['smplx_joint_trunc'])) * net_kps_2d_weight

#             return loss
#         else:
#             # change hand output joint_img according to hand bbox
#             for part_name, bbox in (('lhand', lhand_bbox), ('rhand', rhand_bbox)):
#                 joint_img[:, smpl_x_instance.pos_joint_part[part_name], 0] *= (
#                         ((bbox[:, None, 2] - bbox[:, None, 0]) / cfg.input_body_shape[1] * cfg.output_hm_shape[2]) /
#                         cfg.output_hand_hm_shape[2])
#                 joint_img[:, smpl_x_instance.pos_joint_part[part_name], 0] += (
#                         bbox[:, None, 0] / cfg.input_body_shape[1] * cfg.output_hm_shape[2])
#                 joint_img[:, smpl_x_instance.pos_joint_part[part_name], 1] *= (
#                         ((bbox[:, None, 3] - bbox[:, None, 1]) / cfg.input_body_shape[0] * cfg.output_hm_shape[1]) /
...
#     model = Model(encoder, body_position_net, body_rotation_net, box_net, hand_position_net, hand_rotation_net,
#                   face_regressor)
#     return model